...
**Rules**:
- Do not bulk-replace `sheet-shell.tsx`. It is imported by 18 files; an atomic swap = 18 simultaneous breakages. Per AGENTS.md Lesson 6: one PR per sheet, one verification per PR. Sequencing is tracked in the maintainer's local agent plan named `mobile-sheet-rollout.md`.
...
- Mobile sheet rollout plan: maintainer-local agent plan named `mobile-sheet-rollout.md`
